...
async def register(body: RegisterRequest, db: SqlApiClient = Depends(get_db)):
    ...
    rows = await db.query(f"""
        SELECT user_id FROM User WHERE account = '{body.account}'
    """)
    if rows:
...
async def login(body: LoginRequest, db: SqlApiClient = Depends(get_db)):
    # 1. 查出該 account 的 row
    rows = await db.query(f"""
        SELECT user_id, password FROM User WHERE account = '{body.account}'
    """)
    if not rows:
    ...
    await db.query(f"""
        UPDATE User SET session_id = '{session_id}'
...
async def logout(
    ...
    await db.query(f"""
        UPDATE User SET session_id = NULL

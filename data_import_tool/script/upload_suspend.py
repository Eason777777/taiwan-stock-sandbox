#!/usr/bin/env python3
"""Import cleaned suspension date rows into project_main.suspension_dates."""
...
DEFAULT_FILE = CLEAN_DATA_DIR / "suspend.tsv"
...
PRIMARY_KEY_COLUMNS = {"stock_id", "suspend_start_date"}
...
def read_rows(path: Path):
	with path.open("r", encoding="utf-8-sig", newline="") as file:
		...
		for row in reader:
			...
			suspend_start_date = normalized.get("suspend_start_date", "")
			if not suspend_start_date:
				raise ImportToolError(f"Missing suspend_start_date for stock_id={stock_id}")

			reason = normalized.get("reason", "")
			if not reason:
				raise ImportToolError(f"Missing reason for stock_id={stock_id}, suspend_start_date={suspend_start_date}")

			yield {
				"stock_id": stock_id,
				"suspend_start_date": suspend_start_date,
				"resume_date": normalized.get("resume_date", ""),
				"reason": reason,
			}
...
def main() -> int:
	...
	parser = argparse.ArgumentParser(description="Import cleaned suspension dates into project_main.suspension_dates.")
	parser.add_argument(
		"-f",
		"--filename",
		type=Path,

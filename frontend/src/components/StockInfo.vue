<template>
  <div class="stock-info bg-nature-800 border-[10px] border-nature-500 rounded-lg text-white font-sans flex flex-col gap-6 shadow-2xl">
    <!-- 1. 最上方標頭 -->
    ...
    <!-- 2. 中間第 1 排 -->
    <div class="flex justify-between items-center w-full p-3 rounded-lg ">
      <!-- 左側價格與幅度 -->
      <div class="flex flex-col">
        <span class="text-05 font-bold font-mono" :class="getPriceColorClass(stock.change)">
          {{ formatPrice(stock.price) }}
        </span>
        <span class="text-02 font-bold mt-1" :class="getPriceColorClass(stock.change)">
      ...
      <div class="flex justify-center items-center flex-1">
        <span 
          v-if="hasWarning"
          class="text-red-500 font-bold text-02 border border-red-500/30 px-3.5 py-1 bg-red-500/10 rounded animate-pulse"
        >
          {{ getWarningText() }}
        </span>
      </div>
      ...
      <div class="flex justify-end">
        <button 
          ...
          class="bg-nature-700 hover:bg-nature-600 text-white rounded-[10px] px-5 py-2 border border-nature-600 font-medium cursor-pointer transition-colors duration-200"
        >
          查看公司資訊
        </button>
      </div>
    ...
    <div class="flex justify-between items-center w-full px-1">
      <!-- 左側量價資訊 (兩行排版，支援 NULL) -->
      <div class="flex flex-col gap-1 font-sans text-sm">
      ...
      <!-- 中間持股狀況 -->
      <div class="text-nature-200 font-semibold text-lg text-center flex-1">
        目前持股：<span class="text-yellow-500 font-mono text-xl">{{ holdingsCount }}</span> 張
      </div>
      ...
      <div class="flex justify-end">
        <button 
          ...
          class="bg-yellow-500 hover:bg-yellow-600 active:scale-95 text-nature-900 font-bold rounded-[10px] px-6 py-2 transition-all duration-200 cursor-pointer border-none font-sans"
        >
          前往下單
        </button>
      </div>

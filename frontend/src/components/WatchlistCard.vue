<!-- 自選卡片 -->
<template>
  <div class="watchlist-card w-full max-w-[1280px] min-h-100 sm:min-h-150 flex flex-col items-stretch p-3 sm:p-6 md:p-12.5! gap-3 sm:gap-6 border-3 sm:border-6 md:border-10 border-solid border-nature-500 bg-nature-800 rounded-xl shadow-2xl">
    
    <!-- 1. 上半部：兩個相連按鈕 (新增自選股 & 進入下一階段) -->
    <div class="flex gap-0 w-full">
      <!-- 左：新增自選股按鈕 -->
      <button
        class="group relative flex justify-center items-center flex-1 h-12 sm:h-16 rounded-l-xl! rounded-r-none! bg-yellow-500 hover:bg-yellow-700 transition-colors duration-300 ease-out cursor-pointer border-none outline-none"
      ...
      >
        ...
        <div class="absolute inset-0 flex justify-center items-center transition-all duration-300 ease-out transform scale-100 opacity-100 group-hover:scale-90 group-hover:opacity-0">
          <svg xmlns="http://www.w3.org/2000/svg" width="54" height="54" viewBox="0 0 54 54" fill="none" class="w-9 h-9 sm:w-[54px] sm:h-[54px]">
        ...
        <div class="absolute inset-0 flex justify-center items-center transition-all duration-300 ease-out transform scale-90 opacity-0 text-yellow-500 font-sans font-bold text-02 sm:text-03 group-hover:scale-100 group-hover:opacity-100">
          新增自選股
        </div>
      </button>
      ...
      <!-- 右：進入下一階段按鈕 -->
      <button
        class="group relative flex justify-center items-center flex-1 h-12 sm:h-16 rounded-r-xl! rounded-l-none! bg-green-300 hover:bg-green-700 active:bg-green-900 active:transition-none transition-colors duration-300 ease-out cursor-pointer border-none outline-none disabled:opacity-50 disabled:cursor-not-allowed disabled:pointer-events-none"
      ...
      >
        ...
        <!-- Hover/Pressed 狀態中文字 -->
        <div class="absolute inset-0 flex justify-center items-center transition-all duration-300 ease-out transform scale-90 opacity-0 text-green-300 font-sans font-bold text-02 sm:text-03 group-hover:scale-100 group-hover:opacity-100 group-active:scale-100 group-active:opacity-100 group-active:text-green-700 group-active:transition-none">
          進入下一階段
        </div>
      </button>
    </div>
    ...
    <div class="w-full max-h-96 overflow-y-auto pr-1">
      <div class="w-full flex flex-col gap-2.5 font-sans">
        <!-- 2.1 表頭 (Table Header) -->
        <div class="w-full rounded-t-lg px-1 sm:px-4 py-1.5 sm:py-3 flex text-center font-bold text-01 sm:text-02 text-nature-900 sticky top-0 z-10 bg-nature-200!">
        ...
        <div class="flex flex-col gap-2.5">
          <div
            ...
            :key="stock.id"
            class="group w-full bg-nature-200 hover:bg-nature-600 rounded px-1 sm:px-4 py-2 sm:py-3.5 flex text-center text-01 sm:text-02 items-center transition-colors duration-300 ease-out cursor-pointer"
